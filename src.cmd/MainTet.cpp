// MainTet.cpp
//
// Modify Main.cpp for surface meshes to MainTet.cpp for volume meshes
//
...
//  g++ MainTet.cpp -O3 -o simplifyTet
//To compile for Windows (Visual Studio)
// vcvarsall amd64
// cl /EHsc MainTet.cpp /osimplifyTet
//To execute
//  ./simplifyTet ../data/wall.obj out.obj 0.04
//
...
int simplify(const char* file_path, const char* export_path, float reduceFraction, float agressiveness) {
	...
	if (is_obj(file_path)) {
		SimplifyTet::load_obj(file_path, true);
		printf("loading obj\n");
	}
	else if (is_mz3(file_path)) {
		printf("loading mz3\n");
		SimplifyTet::load_mz3(file_path);
	} else {
		printf("file is not obj or mz3 %s\n", file_path);
		return EXIT_FAILURE;
	}

	if ((SimplifyTet::triangles.size() < 3) || (SimplifyTet::vertices.size() < 3)) {
		printf("triangles size or vertices size less than 3\n");
		return EXIT_FAILURE;
	}

	int target_count =  SimplifyTet::triangles.size() >> 1;
	...
	target_count = round((float)SimplifyTet::triangles.size() * reduceFraction);
	...
	clock_t start = clock();
	printf("Input: %zu vertices, %zu triangles (target %d)\n", SimplifyTet::vertices.size(), SimplifyTet::triangles.size(), target_count);
	int startSize = SimplifyTet::triangles.size();
	SimplifyTet::simplify_mesh(target_count, agressiveness, true);
	//SimplifyTet::simplify_mesh_lossless( false);
	if ( SimplifyTet::triangles.size() >= startSize) {
	...
	if (is_obj(export_path)) {
		...
		SimplifyTet::write_obj(export_path);
	}
	else if (is_mz3(export_path)) {
		printf("exporting mz3\n");
		SimplifyTet::write_mz3(export_path);
	} else {
		printf("export file is not obj or stl %s\n", export_path);
		return EXIT_FAILURE;
	}

	printf("Output: %zu vertices, %zu triangles (%f reduction; %.4f sec)\n",SimplifyTet::vertices.size(), SimplifyTet::triangles.size()
		, (float)SimplifyTet::triangles.size()/ (float) startSize  , ((float)(clock()-start))/CLOCKS_PER_SEC );
	return EXIT_SUCCESS;
}
